...
def get_args_parser():
    parser = argparse.ArgumentParser('LinearClassifier', add_help=False)
...
def prepare_training_objects(args, device, num_classes, total_num_train_steps):
    in_feature_size = 181
    ...
    optimizer = T.optim.SGD(linear_classifier.parameters(), lr=args.init_lr, momentum=0.9,
    ...
    return linear_classifier, optimizer, loss, scheduler
...
def train(args, sampler_name, X_train, y_train, X_test, y_test, num_classes,
    ...
    linear_classifier, optimizer, entropy_loss, scheduler = prepare_training_objects(args, device, num_classes, total_num_train_steps)
    ...
    for epoch in range(args.max_epoch):  # loop over the dataset multiple times
        print('Epoch:', epoch)
        run_train_epoch(train_loader, linear_classifier, optimizer, entropy_loss, metrics_train, device)
        ...
        if (epoch + 1) % args.check_point == 0:
            if best_test_bacc < epoch_bacc:
                ...
                save_best_model(args, output_file_name_bacc, epoch, linear_classifier, optimizer)
            if best_test_f1 < epoch_f1:
                ...
                save_best_model(args, output_file_name_f1, epoch, linear_classifier, optimizer)
            if best_test_mcc < epoch_mcc:
                ...
                save_best_model(args, output_file_name_mcc, epoch, linear_classifier, optimizer)
            if best_test_kappa < epoch_kappa:
                ...
                save_best_model(args, output_file_name_kappa, epoch, linear_classifier, optimizer)
...
def save_best_model(args, output_file_name, epoch, linear_classifier, optimizer):
...
if __name__ == '__main__':
    parser_main = argparse.ArgumentParser('LinearClassifierMain', parents=[get_args_parser()])
